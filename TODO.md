...
## Архитектурные
0. В use-case мы получили фасад телеграм бота. Но:
  - будет ли у него возможность вести диалог? Например мы отправили: "Вы успешно завершили учебу, хотите продолжить?", кнопки "Да", "Нет".
    1. как удалять кнопки после ответа как это принято при работе в разрезе bot-controller-story;
    1. как не мешать основному потоку, ведь например он может в этот момент завести диалог и состояние будет занято определенным контроллером?
    1. в общем нужно не мешать возможному основному процессу взаимодействия с ботом;
    1. нужно дать ему тоже расширенные возможности работы;
