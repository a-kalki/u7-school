...
2. Нужно написать bot-tests (integration, e2e) для модуля onboarding.
3. Кнопка «↩️ Назад» на уровнях глубже главного меню. Должна добавляться на уровне story (каждая story знает свой контекст и явно указывает кнопку возврата). Реализация на уровне роутера откачена — будет в отдельном треке.
