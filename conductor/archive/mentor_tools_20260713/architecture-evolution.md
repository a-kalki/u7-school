...
- **Управление потоками:** подменю «📋 Мои потоки» / «➕ Создать поток» / «👥 Мониторинг»; S02m mentor-режим с lifecycle-кнопками; `CreateStreamStory.handleStart` перенесён на второй уровень (✅).
- **Мониторинг студентов:** логика «Отстаёт» (✅ `StreamDs.computeLagLevel`, `isLaggingFromMedian`), сводка здоровья группы, сортировка, история шагов, bot-level тесты действий ментора в S08.
...
- Карточка: название, ментор, статус, описание.
- `👥 Студенты` → S07 (кнопки-строки: имя → S08 + 🛑 Неактивен / ✅ Завершить / 🔄 Сменить исход).
- `📊 Сводка потока`: счёт по статусам, медиана прогресса, 🛑/⚠️.
- **Внизу** — lifecycle-кнопки: `🚀 Запустить`, `✅ Завершить`, `📁 В архив` (только `completed`).
...
- `mentor_tools_20260713` — S02m mentor-режим, подменю «Инструменты ментора», перенос «Создать поток», мониторинг (сводка, сортировка, отстающие, S08 статистика времени, bot-level тесты, E2E-тесты).
