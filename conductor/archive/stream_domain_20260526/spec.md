...
Контент потока получается через `CourseFacade.getModuleSnapshot()` из `@u7-scl/course`.
...
- Создаётся ментором на основе модуля (`ModuleAr` из `@u7-scl/course`)
- При создании делается **снимок структуры** (`contentSnapshot`) через `CourseFacade.getModuleSnapshot(moduleId)` — полное дерево: проекты → уроки → stepIds
- Поток после создания живёт независимо от изменений модуля
- Параметры: `title`, `description`, `mentorId`, `moduleId`, `startDate`, `telegramGroupId` (ID группы Telegram, опционально), а также поля-снимки из модуля (`goal`, `result`, `rules`, `additional`, `targetAudience`)
- Статус при создании: `enrollment`

### FR-1.5: Запуск и управление статусом потока
- Ментор может перевести поток из `enrollment` в `active` с помощью команды `activate()`
- При активации потока всем студентам, записанным на этот поток, автоматически выдаётся первый шаг из снимка структуры (`currentStepId`)
- Ментор может завершить поток (`completed`), либо отправить его в архив (`archived`)
...
- Содержит полное дерево (тип `ContentSnapshot` из `@u7-scl/course`): `[{ projectId, projectTitle, lessons: [{ lessonId, lessonTitle, stepIds[] }] }]`
...
- `canEnroll`: только пользователи с ролями `GUEST` или `CANDIDATE`. Пользователь с ролью `STUDENT` (уже обучается на другом потоке) или `MENTOR` не может записаться на поток. При зачислении роль пользователя автоматически меняется на `STUDENT`.
...
Stream {
  ...
  status: StreamStatus
  telegramGroupId?: string // ID группы Telegram (комната для учёбы)
...
| `activate-stream`| `activate()` | Запустить поток (статус active) |
| `complete-stream`| `complete()` | Завершить поток (статус completed) |
| `archive-stream` | `archive()` | Архивация потока (статус archived) |
...
- [ ] `StreamAr.create()` создаёт поток с корректным `contentSnapshot`, статус `enrollment`, `telegramGroupId` опционально сохраняется
- [ ] `StreamAr.activate()` успешно переводит поток в статус `active`, `complete()` — в `completed`, `archive()` — в `archived`
...
- [ ] `StreamPolicy` проверяет права по ролям и авторству, включая новую политику `canEnroll`
