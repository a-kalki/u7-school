...
## Решения владельца по сверке ui-spec

«Путь пользователя» в questionnaire/ui-spec.md упоминал несуществующий вход
«запись на модуль» (create-module-wish — мгновенная фиксация, без анкеты) и не
описывал фактический вход — запись на курс. Решение владельца (2026-09-11):
описать как в коде — отдельного приглашения сверх реализованного не вводить,
анкета открывается только по клику. Внесено: S01b —
«Приглашение при записи на курс» (apply → pending → приглашение →
кнопка-мост), W02 в courses/ui-spec.md уточнён (приходит приглашение,
анкета открывается кнопкой). S01 оставлен как вход likert-анкет
(send-likert-invite, из бота не вызывается).
