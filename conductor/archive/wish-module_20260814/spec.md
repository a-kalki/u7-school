...
Создаётся новый модуль **`@u7-scl/wish`** («Желание»), который фиксирует **желание пользователя** (например пройти курс). Модуль заменяет старый `@u7-scl/onboarding` (весь код анкетирования из onboarding удаляется — анкеты теперь ведёт `questionnaire`).
