...
## Phase 1: Создание структуры packages/user [x] [checkpoint: c75b9b2]

- [x] Task: Создать `packages/user/package.json` с зависимостями `@u7/core`, `valibot`
- [x] Task: Создать `packages/user/tsconfig.json`, наследующий базовую конфигурацию
- [x] Task: Создать директории `packages/user/src/{domain,api,ui}/`
- [x] Task: Создать `packages/user/src/index.ts` с публичными экспортами
- [x] Task: Conductor - User Manual Verification 'Phase 1' (Protocol in workflow.md)

## Phase 2: Перенос домена User из packages/course [x] [checkpoint: fc89e4d]

- [x] Task: Перенести `domain/user/user-ar.ts` → `packages/user/src/domain/user/user-ar.ts`
- [x] Task: Перенести `domain/user/user.ts` → `packages/user/src/domain/user/user.ts`
- [x] Task: Перенести `domain/user/roles.ts` → `packages/user/src/domain/user/roles.ts`
- [x] Task: Перенести `domain/user/user-policy.ts` → `packages/user/src/domain/user/user-policy.ts`
...
- [x] Task: Обновить `packages/course/src/index.ts` (удалить экспорты user, добавить зависимость от `@u7/user` при необходимости)
- [x] Task: Conductor - User Manual Verification 'Phase 2' (Protocol in workflow.md)

## Phase 3: Реализация API-модуля auth [x] [checkpoint: 865f6b0]

- [x] Task: Создать интерфейс `UserRepository` в `packages/user/src/api/user-repository.ts`
- [x] Task: Реализовать `InMemoryUserRepository` в `packages/user/src/api/user-repository-inmemory.ts`
- [x] Task: Написать тесты для `InMemoryUserRepository`
- [x] Task: Создать схему `CreateUserCommandSchema` в `packages/user/src/api/commands/create-user-command.ts`
...
- [x] Task: Реализовать `GetUserByTelegramIdUc`
    ...
    - [x] Subtask: Реализовать UseCase
    - [x] Subtask: Убедиться, что тесты проходят
- [x] Task: Создать `AuthApiModule extends Module` в `packages/user/src/api/auth-module.ts`
...
- [x] Task: Создать `AuthAutoUiModule extends AutoUiModule` в `packages/user/src/ui/auto-ui/auth-auto-ui-module.ts`
- [x] Task: Экспортировать `AuthAutoUiModule` из `packages/user/src/index.ts`
...
- [x] Task: Обновить `apps/u7-cli/package.json` — добавить зависимость `@u7/user`
- [x] Task: Conductor - User Manual Verification 'Phase 4' (Protocol in workflow.md)

## Phase 5: Финализация и проверка качества [x] [checkpoint: f1f9ae4]

- [x] Task: Убедиться, что `packages/user` собирается (`tsc --noEmit`)
