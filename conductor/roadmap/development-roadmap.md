...
| II | Система сбора метрик студента | [metrics-system.md](./metrics/metrics-system.md) | ◐ частично (Релиз 3 готов, Релиз 4 — нет) |
...
[metrics-conception.md](./metrics/metrics-conception.md).
...
- **Этап A — механизм уведомлений** `userFacade.notify` (трек `user-notify_20260902`,
  в архиве) — ✅ реализован: событие `user.notified` + контроллер user; перенесены
  только чистые уведомления (уход/снятие, поздравление о завершении, набор
  желающим), дубль зачисления удалён. Кнопочные проактивы (бездействие,
  «Следующий модуль») и анкеты остаются на подписках до этапа B.
