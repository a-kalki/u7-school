...
> Дополнения независимого ревью (хлебные крошки, гашение при любой смене
> экрана/диалога, константы приложения, delegate в командах) — 2026-09-09, §10.17–20.
...
- **Хлебные крошки** — UX-концепция навигации: история пройденных экранов
  остаётся в чате — каждый прошлый экран гасится (клавиатура снята, маркер
  выбора при известной кнопке), текущий — единственный живой. Пользователь
  видит свой путь. Механика транспорта при любой смене экрана/диалога;
  стори про это не думает — ответила экраном, прежний экран погаснет сам.
...
- **Маркер выбора** — строка `—————\nВы выбрали: <текст кнопки>`, которой
  транспорт сопровождает гашение (retire) прошлого экрана при ЛЮБОЙ смене
  экрана/диалога — не только при send-пути по кнопке. Текст кнопки ищется в
  клавиатуре гасимого экрана по коду нажатой кнопки; команды (`/start`)
  гасят без маркера — кнопки не было.
...
- **И2. Один живой экран.** Ровно одно сообщение в чате несёт клавиатуру —
  активный экран диалога. Смена экрана/диалога **всегда** гасит клавиатуру
  прежнего (retire, «хлебные крошки» §1) — включая ответы без `screen`
  (notify-only) после смены диалога: мёртвая клавиатура не остаётся висеть.
...
2a. **Гашение без `screen`.** Ответ без `screen`, после которого прежний
   активный экран перестал принадлежать диалогу (`ownerSeq ≠ seq` — смена
   диалога мостом/командой/delegate), тоже гасит его (retire: снять
   клавиатуру, маркер при известном коде кнопки). «Хлебные крошки» полны
   при любом ответе стори — стори не обязана возвращать экран, чтобы
   погасить прежний.
...
и shortIds — трек 6 (выполнен): порт `BotSessionRepo` в core/ui, реализация
`JsonBotSessionRepo` на JsonFileRepo (dbDir/bot/{sessions,short-ids}.json),
sинхронная запись после каждого апдейта, restore до старта polling/webhook.
...
3. Деплой: старые кнопки в чатах один раз умирают с alert «нажмите /start» —
   приемлемо.
4. ~~`seq` в памяти с нуля после рестарта — старые кнопки мертвы~~ — закрыто
   треком 6: сессии и shortId-мапа переживают рестарт
   (`JsonBotSessionRepo`), старые кнопки остаются нажимаемыми.
...
### Дополнения независимого ревью (2026-09-09)

17. **«Хлебные крошки» — целевой UX навигации.** История пройденных экранов
    остаётся в чате: каждый прошлый экран гасится (клавиатура снята, маркер
    выбора при известной кнопке), текущий — единственный живой. Пользователь
    видит свой трек. Стори не управляет гашением — это механика транспорта
    при любой смене экрана/диалога (И2, §5.2a); переходы между стори
    (retire+send) не минимизируются искусственно — следы пути осознанно
    сохраняются.
18. **Константы приложения — единый модуль.** Системные коды кнопок и якоря
    диалогов (`app:main-menu`, `app:help`, `app/menu`, `app/invite`) — в
    одном файле констант приложения; транспорт, прикладной uiApp и `Routes`
    импортируют их явно. Снимает хардкод `'app:'` в транспорте и дубли
    строк между слоями.
19. **`delegate` в stop-ответах команд исполняется ядром** (симметрично
    `handleCallback`): команда с экраном открывает диалог целевой стори
    (`/tasks` → `tasks/list`) — `/help`, `/cancel` и текстовый ввод
    адресуются экрану, который видит пользователь; кейс «команда до
    /start» перестаёт молчать (диалог открывается командой легально).
20. **Экраны ошибок — с кнопкой выхода.** `handleError` экранов ядра получает
    точку расширения (кнопка «⬅️ Меню» задаваемая приложением, для задач —
    «⬅️ Мои задачи»); `errorNotify` — без изменений (реплика-переспрос,
    экран и ввод не трогает).
...
| `apps/u7-bot/src/shared/app-codes.ts` | константы приложения: системные коды кнопок (`app:main-menu`, `app:help`) и якоря диалогов (`app/menu`, `app/invite`) — единый источник для транспорта, прикладного uiApp и `Routes` |
...
3. ~~Персистентность сессий + shortIds: хранилище и формат~~ — решено (трек 6):
   порт `BotSessionRepo` (core/ui/bot), JSON-файлы через `JsonFileRepo`
   (сессии: ключ — telegramId; shortId: полный ключ с суффиксом коллизии).
   Повреждение файла — fail-fast (JsonFileRepoError); ошибка записи —
   warn-лог с продолжением в памяти.
