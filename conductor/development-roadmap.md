...
| V | Система задач и уведомлений | [tasks-system-implementation.md](./tasks-system-implementation.md) | ◐ этап A расписан (трек `user-notify_20260902`), этап B — заготовка |
...
Статус: концепция согласована. Точка входа в реализацию —
[tasks-system-implementation.md](./tasks-system-implementation.md):
- **Этап A — механизм уведомлений** `userFacade.notify` (трек `user-notify_20260902`):
  событие `user.notified` + контроллер user; перенос только чистых уведомлений
  (уход/снятие, поздравление о завершении, набор желающим), дубль зачисления
  удаляется. Кнопочные проактивы (бездействие, «Следующий модуль») и анкеты
  остаются на подписках до этапа B.
- **Этап B — модуль задач**: сущности из концепции, список «Мои задачи» (`/tasks`),
  миграция бездействия и анкет-приглашений на задачи. Архитектурная концепция и
  декомпозиция на треки — отдельная сессия.
