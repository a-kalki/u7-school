...
> **Примечание (актуализация):** описанная ниже структура — историческая точка старта. Движок анкет давно извлечён в пакет `questionnaire` (треки 2.4–2.5 выполнены); пакет `onboarding` удалён — его зона ответственности (желания пройти курс) передана пакету `wish` (см. трек wish-lifecycle). Раздел сохранён как история проектирования.

Модуль `onboarding` содержал полноценный движок анкет:
