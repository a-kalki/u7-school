...
Сейчас S02 смешивает curious-кнопки и менторские lifecycle-действия, а «👥 Студенты» доступна всем, но данные видит только ментор. Целевая модель — **два режима карточки потока**, живущих в разных ветках навигации:
...
**Прозрачность студентов (важное решение):** S07 (список) и S08 (карточка студента) доступны **всем ролям** — curious видит прогресс, статусы, историю шагов, динамику и исходы студентов (кто как учился, кого отчислили за халтуру, кто каких успехов достиг). Кнопки **действий** над студентом (`mark-abandoned`, `complete-student`) — только ментор потока / ADMIN.

**Распределение по трекам:**
- `curious_showcase_20260708` — curious-режим S02, «Программа/Детали на любом статусе», кнопка «Курсы», прозрачность S07/S08 (снятие role-gate).
