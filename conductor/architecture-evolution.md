...
Два role-gated пространства в главном меню (см. §2.11). Принцип разделения веток: curious-ветка «Потоки» не содержит менторских инструментов вообще — управление потоком живёт только в «Инструментах ментора».

**🛠️ Инструменты ментора** (`MENTOR`, трек `curious_showcase_20260708`):
- Подменю: «📋 Мои потоки» (потоки, где я ментор), «➕ Создать поток» (S09), «👥 Мониторинг» (быстрый вход в активные потоки → S07).
- `Мои потоки` → карточка потока в **mentor-режиме** (S02m): lifecycle-действия (`🚀 Запустить` / `✅ Завершить` / `📁 В архив`) + вход в мониторинг (`👥 Студенты` → S07/S08 с кнопками действий).
...
**S02 — curious-режим** (ветка «📚 Потоки», всем ролям):
...
- Гостевая часть (только не-владелец): `📝 Записаться` (`enrollment`), `🔔 Уведомить о наборе` / `🔔 Ждать новый набор` (`active`/`completed`).
...
- `📊 Сводка потока`: счёт по статусам (active/advanced/not_advanced/abandoned/enrolled). Медиана, «отстающие», сортировка — трек `mentor_monitoring_20260708`.

**Принцип:** curious-ветка «Потоки» не содержит менторских инструментов. Управление потоком — только через «Инструменты ментора». Дублирования кнопок между ветками нет.
...
Трек: `curious_showcase_20260708` (curious-режим S02, «Программа/Детали на любом статусе», кнопка «Курсы», Инструменты ментора + S02m). Переработка **контента** S07/S08 (сводка здоровья, сортировка отстающих, история шагов, динамика) — трек `mentor_monitoring_20260708`.

### 2.11. Главное меню: 5 веток

Целевая структура после треков 5/4:

```
📚 Программы          (всем)        каталог курсов + структура (этапы/модули/уроки/заголовки шагов)
📚 Потоки             (всем)        curious-витрина потоков: программа, детали, студенты, запись
...
- **Программы** (S00→S00a→S00b): drill-down структуры курса на curious-уровне ContentPath (заголовки и объём видны, тела шагов — нет). Переход к потокам модуля курса.
- **Потоки** (S01→S02 curious-режим): только curious-функциональность, без менторских инструментов (см. §2.10).
- **Инструменты ментора**: `Мои потоки` → S02m (mentor-режим), `Создать поток` → S09, `Мониторинг` → S07.
...
| 6 | `mentor_monitoring_20260708` | 1, 2, 5 | 3 |

```
A (AUTHOR) → 0 (Курс) ─┐
                        ├─→ 2 (ContentPath) ─┬─→ 4 (Навигация студента)
          1 (ЖЦ) ────────┤                     ├─→ 5 (Витрина + Инструменты ментора)
                        ├─→ 3 (Gating)        │
                        └─→ 6 (Мониторинг) ◄──┘
```

> Трек 6 зависит от трека 5: прозрачность S07/S08 (доступ всем ролям) вводится в треке 5, переработка контента S07/S08 — в треке 6.
