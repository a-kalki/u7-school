...
- Домен `course`. Агрегат `Course`: ordered `phases: { id, title, track: 'tech'|'business', moduleIds[] }[]`.
...
### 2.8. «Инструменты ментора» — пространство ментора
- Первое-level-меню для MENTOR/ADMIN: «🛠️ Инструменты ментора» → «➕ Создать поток», «📋 Мои потоки» и т.д.
- `CreateStreamStory.handleStart` перевести на второй уровень.
...
- **Задача:** обновить сломанные старые тесты (статусы `expelled`→`abandoned` и т.д.) + добавить новые сценарии по каждому треку.
