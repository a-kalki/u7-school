...
- Содержимое карточки + панель управления: `🚀 Запустить`, `✅ Завершить`, `📁 В архив` (только для `completed`).
- `👥 Студенты` → S07 (кнопки-строки с маркерами 🛑/⚠️/✅) → S08 **с кнопками действий ментора** (`⚠️ Неактивен`, `✅ Завершить`/`🔄 Сменить исход`) и историей шагов со статистикой времени (🏃 <1мин / ⚡ 1-5мин / 📝 5-15мин / 📚 >15мин).
- `📊 Сводка потока`: счёт по статусам (active/advanced/not_advanced/abandoned/enrolled). Медиана прогресса. Число отстающих (⚠️) и критических (🛑). Сортировка: 🛑 → ⚠️ → ✅ по прогрессу.
- **Вход в мониторинг — только через карточку потока**, отдельной кнопки «Мониторинг» в подменю нет.
...
- **Релиз 2 «UX ядра»** ✅ завершён: треки 3 (gating) + 4 (навигация студента) + 5 (витрина для любопытного).
  - Миграция данных в проде: **не требуется**. Все изменения — код UI и контроллеров, структура фикстур не менялась.
