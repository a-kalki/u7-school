# Track metrics-onboarding-migration_20260810 — Трек 2.5

- [Спецификация](./spec.md)
- [План реализации](./plan.md)
- [Метаданные](./metadata.json)

## Контекст

- [Система сбора метрик](../../metrics-system.md) — родительский документ
- [2. Questionnaire + EventBus](../../metrics-questionnaire-and-events.md) — техническая спецификация
- [Трек 2.4a — модуль questionnaire](../metrics-questionnaire_20260810/spec.md) — API фасада
- [Трек 2.1 — EventBus](../metrics-eventbus_20260810/spec.md) — подписка на события
- [Дорожная карта](../../development-roadmap.md) — Релиз 3
