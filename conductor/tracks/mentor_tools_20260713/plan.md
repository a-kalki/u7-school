...
- [ ] Task: Написать тесты: каждый студент — ряд кнопок (имя + Неактивен + Завершить/Сменить исход); маркеры 🛑/⚠️/🏃/✅
- [ ] Task: Реализовать сводку (медиана, счёт по статусам, отстающие/критические) и сортировку (🛑 → ⚠️ → 🏃 по прогрессу → ✅)
- [ ] Task: Кнопка с именем студента → S08 (карточка со статистикой)
