...
- [x] Task: Написать тесты: >4 дней → ⚠️ «Отстаёт»; >7 дней → 🛑 «Критическое»; отставание от медианы 30%+
- [x] Task: Реализовать в MonitorStory два уровня отставания (completedAt, медиана)
- [ ] Task: Conductor - Ручная верификация 'Логика Отстаёт'

## Фаза 5: Студенты кнопками-строками с действиями + сводка + сортировка (S07)

- [x] Task: Написать тесты: каждый студент — ряд кнопок (имя + Неактивен + Завершить/Сменить исход); маркеры 🛑/⚠️/🏃/✅
- [x] Task: Реализовать сводку (медиана, счёт по статусам, отстающие/критические) и сортировку (🛑 → ⚠️ → 🏃 по прогрессу → ✅)
- [x] Task: Кнопка с именем студента → S08 (карточка со статистикой)
