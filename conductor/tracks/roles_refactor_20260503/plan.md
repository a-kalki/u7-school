...
- [x] Task: Обновить тесты для `UserSchema` — заменить `role` на `roles: Role[]`, добавить новые тесты
...
- [x] Task: Обновить тесты для `UserAr` с множественными ролями
    - [ ] Обновить существующие тесты: `role` → `roles` (массив)
    - [ ] Новый тест: создание пользователя с несколькими ролями
    - [ ] Новый тест: изменение ролей через метод агрегата
- [x] Task: Обновить тесты для `UserPolicy` с множественными ролями
...
- [x] Task: Обновить тесты для `UserCreatingUc` с `roles: Role[]`
    - [ ] Обновить существующие тесты: `role` → `roles` (массив)
    - [ ] Новый тест: ошибка при пустом массиве ролей
- [x] Task: Реализовать `UserSchema`: заменить `role` на `roles: v.array(RoleSchema)` с `minLength(1)`
- [x] Task: Реализовать `UserAr`: поддержка `roles: Role[]` (конструктор, create, методы)
- [x] Task: Реализовать `UserPolicy`: проверка прав по массиву ролей
- [x] Task: Реализовать `CreateUserCommand` и схему: `role` → `roles`
