...
- [x] Task: Написать падающие тесты UC `drop-student`: самовыход (who=self) → `userFacade.notify` ментору «🚪 Студент X покинул учёбу с потока «Y» по собственному желанию.» (704c70d)
- [x] Task: Реализовать notify в UC `drop-student` (резолв имени студента и названия потока, текст FR-6) (704c70d)
