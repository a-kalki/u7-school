...
| (этот коммит) | Обновлены `ui-spec.md` (learning, courses); создан `summary.md` |
...
2. **Проактивы без кнопок:** ветки 7a/7b `student.completed` переведены с `send()` (кнопки на каталог) на `proactiveSender.notify` с подсказкой меню `/start → 📖 Программы курсов`. Обработчик `CourseCatalogStory.wish` сохранён как мост. Безкнопочные 7c/7d — как и раньше, через `userFacade.notify` из UC `complete-student`.
...
- Существенных отклонений нет. Единственный доп. фикс в фазе 2: перестановка импорта `MenuButton` в `hub.ts` (biome organizeImports, db5aca61).
...
- Кнопки `➡️ Следующий модуль` / `🔁 Пройти модуль снова` больше не рендерятся (уведомления текстовые); обработчик-мост `course-catalog:wish` жив, задокументирован в courses/ui-spec.md.
