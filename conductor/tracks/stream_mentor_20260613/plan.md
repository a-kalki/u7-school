...
## Фаза 1: US-6 — Реальный список модулей и превью

- [ ] Task: Написать тест: CreateStreamStory показывает список модулей ментора
  - [ ] Мок API возвращает модули (moduleId, title)
  - [ ] Проверить, что в ответе есть inline-кнопки с названиями модулей

- [ ] Task: Реализовать загрузку модулей в wizard
  - [ ] В `#handleModuleMessage` запрашивать модули через `this.api.execute('list-modules', {})`
  - [ ] Формировать inline-кнопки: `{ text: module.title, code: 'create-stream:module:<moduleId>' }`

- [ ] Task: Написать тест: CreateStreamStory показывает превью перед созданием

- [ ] Task: Реализовать шаг превью (шаг 5 → шаг 6)
  - [ ] После ввода группы показать карточку: название, описание, дата, группа
  - [ ] Кнопки: `[✅ Создать]`, `[⬅️ Изменить]`
  - [ ] «Создать» вызывает `this.api.execute('create-stream', ...)` и освобождает input
