...
- [ ] Task: S02 my-reviews — абстрактное интро, нумерованные мини-карточки
  (тексты по парам роль-судьба), `Метрики: M/K (N дн.)`, разделители,
  кнопки внизу с номером карточки
- [ ] Task: S03/S06 campaign — шапка «Отзывы — поток», вводный абзац по парам
  роль-судьба, `📊 Прогресс` построчно, роли адресатов
  «Ментор/Одногруппник/Студент»
- [ ] Task: S05/S04 — подсказки по парам (мягкий шаблон двух секций) + блок
  принципов; S05i — справка «Как писать отзыв» (callback без выхода
  из awaitInput)
- [ ] Task: Фикстуры — имена Андрей/Марина/Олег (333/777/888), ментор c2 →
  44444444, c4 `participants: []`; пересмотр ожиданий юнит/e2e
