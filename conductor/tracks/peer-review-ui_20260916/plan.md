...
> **Предусловие:** завершён трек [peer-review-domain_20260916](../peer-review-domain_20260916/plan.md).
...
- [ ] Task: Каркас контроллера peer-review + стори кампании: S03 список адресатов (`✅`-признаки, шапка с остатком дней, правила видимости ментора) через `get-campaign-recipients`
    - [ ] Red: тесты S03 (роли кнопок, ✅, заголовок)
    - [ ] Green: реализация
- [ ] Task: S05 ввод отзыва — `awaitInput`, тексты-подсказки по направлению/статусу автора (спека S05), валидация 10–3500 (`errorNotify`, контекст не теряется), `⏭️ Пропустить`
...
- [ ] Task: `menuButtons` «💬 Отзывы» — видимость по фасаду `hasLiveCampaigns` (декларативно, без обработчиков)
    ...
    - [ ] Green: реализация
- [ ] Task: Экран S02 — список живых кампаний с прогрессом M/K и остатком дней (`get-my-campaigns`), выбор кампании → S03
    - [ ] Red: тесты
    - [ ] Green: реализация
...
- [ ] Task: Подписка на `campaign.created` → приглашения участникам: три шаблона текстов (студент advanced/not_advanced; enrolled→abandoned/neverStarted-ветка спеки; ментор), механика `ProactiveSender.invite` (ФР-6), кнопка `💬 Отзывы` → S03 кампании
    - [ ] Red: тесты подписки (адресация, тексты по ролям/статусам, одно приглашение на участника)
    - [ ] Green: реализация
- [ ] Task: Conductor - User Manual Verification 'Фаза 4' (Protocol in workflow.md)

## Фаза 5. Просмотр S07 и карточка потока

- [ ] Task: Стори просмотра S07 — `list-scope-reviews`, группировка по адресатам со статусами-снапшотами, пагинация (хелпер Фазы 1), `⬅️ Назад к потоку`
