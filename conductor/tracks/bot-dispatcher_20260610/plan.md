...
- [x] Task: Создать registerDispatcher(bot, controllers, userFacade, logger) (07ac192)
...
- [x] Task: Написать тесты на диспетчер (07ac192)
    - [x] Тест: /start агрегирует кнопки
    - [x] Тест: callback маршрутизируется по префиксу
    - [x] Тест: неизвестный префикс → ошибка
    - [x] Тест: /cancel с активным обработчиком
    - [x] Тест: /cancel без активного обработчика
    - [x] Тест: captureInput → последующие сообщения идут в контроллер
    - [x] Тест: releaseInput → сессия очищается
    - [x] Тест: чужой callback при активном обработчике → отказ
    - [x] Тест: таймаут → handleTimeout
