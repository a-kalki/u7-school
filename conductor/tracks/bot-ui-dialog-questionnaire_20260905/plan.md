...
- [ ] Task: **Инвентаризация кнопок/реплик** fill, invite (`render.ts`) из `ui-spec.md` и кода → зафиксировать в падающих тестах с точными keyboard-ассертами (текст + код каждой кнопки); протокол «миграция без потери функциональности» — решение владельца, ревью трека 3 (см. spec)
