...
## UI: auto-ui/module.ts

```typescript
import { AutoUiModule } from "@u7/core";

export class UserAutoUiModule extends AutoUiModule<
  UserModuleMeta
  // тут могут быть типы AppResolver, ModuleResolver
> {
	readonly name = "user" as const;
}
```
...
1. В тестах используй реальные реализации (имплементации) репозиториев, фасадов. Твоя задача убедиться что имплементации нормально работают. 
