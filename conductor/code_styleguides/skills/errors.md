...
1. **Доменные ошибки** — ошибки нарушения доменных правил.
2. **Ошибки приложения** — ошибки на уровне приложения.

## Атомарные vs Union

- **Атомарные** (`<Name>UcError`) — каждая ошибка, это один конкретный тип ошибки с фиксированным `name`, `kind`, `level`.
...
Ошибки тестируются косвенно — через тесты UC, которые их выбрасывают. Не нужен отдельный unit-тест на тип ошибки.
