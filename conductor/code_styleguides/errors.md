...
Приложение обязано возвращать объект ошибки `AppError` обернутый в `AppException` в запросах извне. Разрешается кидать обычный `throw Error(...)` только в момент загрузки приложения;
