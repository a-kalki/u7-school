...
Доменные объекты можно импортировать и использовать из любого слоя (api, ui) и из других доменных модулей напрямую — без UC, без `appApi.execute()`. Исключение: если операция **меняет состояние** или **читает из БД** — только через UC/фасад.

### Что можно импортировать напрямую

- **Агрегат (Ar)** — фабрики (`create()`) и методы изменения состояния
- **Политика (Policy)** — синхронные проверки прав
- **Доменный сервис (Ds)** — чистые функции, координирующие несколько агрегатов
- **Схема (Schema)** — валидационные схемы Valibot
- **Entity-типы** — `User`, `Course`, `Student`, `Stream` и т.д.

### Что нельзя импортировать напрямую

- **Repo** (интерфейсы и реализации) — только внутри пакета-владельца
- **UseCase** другого модуля — только через `execute()` или фасад
