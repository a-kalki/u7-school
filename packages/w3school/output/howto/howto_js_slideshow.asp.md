...
[](javascript:void\(0\) "slide 1")[](javascript:void\(0\) "slide 2")[](javascript:void\(0\) "slide 3")[](javascript:void\(0\) "slide 4")

[Try it Yourself »](https://www.w3schools.com/howto/tryit.asp?filename=tryhow_js_slideshow)
...
```javascript
let slideIndex = [1,1];/* Class the members of each slideshow group with different CSS classes */let slideId = ["mySlides1", "mySlides2"]showSlides(1, 0);showSlides(1, 1);function plusSlides(n, no) {  showSlides(slideIndex[no] += n, no);}function showSlides(n, no) {  let i;  let x = document.getElementsByClassName(slideId[no]);  if (n > x.length) {slideIndex[no] = 1}  if (n < 1) {slideIndex[no] = x.length}  for (i = 0; i < x.length; i++) {    x[i].style.display = "none";  }  x[slideIndex[no]-1].style.display = "block";}
```

**Tip:** Also check out [How To - Slideshow Gallery](howto_js_slideshow_gallery.asp.html) and [How To - Lightbox](howto_js_lightbox.asp.html).