...
const mockStudent = {
  uuid: studentId, streamId, userId,
  enrolledAt: isoNow(), status: 'active' as const,
  currentStepId: 's1', steps: [], createdAt: isoNow(),
};

const mentor = { uuid: mentorId, name: 'M', telegramId: 1, roles: [Role.MENTOR], createdAt: isoNow() };
const student = { uuid: userId, name: 'S', telegramId: 3, roles: [Role.STUDENT], createdAt: isoNow() };
const otherStudent = { uuid: 'other', name: 'O', telegramId: 4, roles: [Role.STUDENT], createdAt: isoNow() };

const mockStream = {
  uuid: streamId, title: 'T', description: 'D', mentorId,
...
const baseResolve = (u: any = student, s: any = undefined) => ({
  streamRepo: { getByUuid: mock(() => Promise.resolve(s)), save: mock(() => Promise.resolve()), getAll: mock(() => Promise.resolve([])) },
  streamStudentRepo: {
...
describe('GetStudentProgressUc', () => {
  ...
  test('чужой студент не видит чужой прогресс', async () => {
    ...
    await expect(uc.execute({ studentId }, 'other')).rejects.toThrow('Недостаточно прав');
  });
