 ...
 */
import { SPLIT_CONFIG, type VideoSplit, type PartConfig } from "./split-config";
...
function buildTitleArgs(input: string, caption: string, output: string): string[] {
...
async function splitPart(
  ...
  console.log(`    резка -ss ${part.start}${part.end ? ` -to ${part.end}` : ""} ...`);

  const cutArgs = buildCutArgs(video.source, part.start, part.end, tempPath);
  const cutProc = Bun.spawn(["ffmpeg", ...cutArgs], { stdout: "pipe", stderr: "pipe" });
...
async function main() {
  ...
  console.log(`🎬 Разрезаю ${SPLIT_CONFIG.length} видео на ${totalParts} частей с титрами\n`);
