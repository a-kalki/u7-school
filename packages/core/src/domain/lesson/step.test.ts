...
import { StepSchema, type TextStep, type CodeStep, type FileStep } from "./step";
...
describe("Схема шага (Step) — text | code | file", () => {
  ...
  test("должна принимать code-шаг с контентом и языком", () => {
    const s: CodeStep = { ...baseFields, kind: "code", content: "Пояснение", code: "print(1)", language: "python" };
    expect(v.safeParse(StepSchema, s).success).toBe(true);
  });
  ...
  test("должна отклонять code-шаг без кода", () => {
    expect(v.safeParse(StepSchema, { ...baseFields, kind: "code" }).success).toBe(false);
  });

  test("должна отклонять file-шаг без файла", () => {
    expect(v.safeParse(StepSchema, { ...baseFields, kind: "file" }).success).toBe(false);
  });
});
