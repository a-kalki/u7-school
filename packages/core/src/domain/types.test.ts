...
function _assertUcMetaHasDescription<T extends { description: string }>(
	_meta: T,
) {}
_assertUcMetaHasDescription({} as UcMeta);

function _assertUcMetaHasArMeta<
	T extends { arMeta: { name: string; label: string } },
>(_meta: T) {}
_assertUcMetaHasArMeta({} as UcMeta);

function _assertUcMetaHasRequiresAuth<T extends { requiresAuth: boolean }>(
	_meta: T,
) {}
_assertUcMetaHasRequiresAuth({} as UcMeta);

function _assertUcMetaHasType<T extends { type: "command" | "query" }>(
	_meta: T,
) {}
