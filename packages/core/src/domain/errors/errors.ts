...
export type NotFoundError<N extends string = string, P = unknown> = {
  name: N;
  level: 'domain';
  kind: 'not-found';
  message: string;
...
export type ConflictError<N extends string = string, P = unknown> = {
  name: N;
  level: 'domain';
  kind: 'conflict';
  message: string;
...
export type AccessDeniedError<N extends string = string, P = unknown> = {
  name: N;
  level: 'domain';
  kind: 'access-denied';
  message: string;
...
export type BadRequestError<N extends string = string, P = unknown> = {
  name: N;
  level: 'api';
  kind: 'bad-request';
  message: string;
...
export type UnauthorizedError<N extends string = string> = {
  name: N;
  level: 'api';
  kind: 'unauthorized';
  message: string;
...
export type ValidationError<N extends string = string, P = unknown> = {
  name: N;
  level: 'domain';
  kind: 'validation';
  message: string;
...
export type InputValidationError = ValidationError<
  ...
  ValidationIssues
>;
export type OutputValidationError = InternalError<
  'OUTPUT_VALIDATION_ERROR',
  ValidationIssues,
  'domain'
>;
export type AuthError = UnauthorizedError<'UNAUTHORIZED_ERROR'>;
export type NoCommandFoundError = BadRequestError<
  'NO_COMMAND_FOUND',
  { commandName: string; moduleName: string }
>;
