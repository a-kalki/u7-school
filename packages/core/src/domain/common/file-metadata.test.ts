...
describe("FileMetadataSchema", () => {
  ...
  test("отклоняет невалидный uuid", () => {
    expect(v.safeParse(FileMetadataSchema, { ...valid, uuid: "bad" }).success).toBe(false);
  });

  test("отклоняет пустое имя", () => {
    expect(v.safeParse(FileMetadataSchema, { ...valid, name: "" }).success).toBe(false);
  });

  test("отклоняет пустой mimeType", () => {
    expect(v.safeParse(FileMetadataSchema, { ...valid, mimeType: "" }).success).toBe(false);
  });

  test("отклоняет отрицательный size", () => {
    expect(v.safeParse(FileMetadataSchema, { ...valid, size: -1 }).success).toBe(false);
  });
});
