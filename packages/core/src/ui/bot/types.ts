 ...
 * и диалоговых реплик (ФР-5). Рендер — таблица транспорта:
 * `notify` → 🔔, `info` → ℹ️, `warn` → ⚠️.
 */
 ...
 *
 * Проактивный запуск диалога удалён: невоспроизводим синтаксически — только
 * notify без кнопок и без влияния на сессию (инвариант ИЗ).
 */
export interface ProactiveSender {
   ...
   * временный диалог-якорь `app/invite` (seq = 1) — кнопки живы до смены
 ...
 *
 * `keyboard` имеет смысл только у активного экрана диалога; в `notify`
 * (реплике поверх диалога) клавиатуры нет по построению (типом).
 */
...
export interface DialogResponse {
  ...
  /**
   * Реплика поверх диалога (ФР-5): без клавиатуры, сессию и экран
   * не трогает (ожидание ввода живёт — переспрос). Вид — та же таблица
   * 🔔/ℹ️/⚠️, что у проактива; по умолчанию `info` (ℹ️).
   */
...
/**
 * Реакция на команду в трёхуровневом pipe (ФР-4): единый тип для стори
 * и контроллера — ядро имён команд не знает.
 *
 * `pass` — «не моё»; `continue` — «моё», обход продолжается, вклад — только
 * текст-нотис (склейка \n\n); `stop` — терминал, `response` — итоговый ответ,
 * накопленные нотисы ставятся info-репликой над ним. Дефолты команд —
 * уровень приложения (весь pipe вернул pass).
 */
