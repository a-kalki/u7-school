...
describe("command-parser", () => {
	...
	it("должен парсить глобальные команды приложения", () => {
		...
		expect(parser.parse("/modules")).toEqual({
			type: "app",
			command: "modules",
		});
	...
	it("должен парсить команды уровня модуля", () => {
		expect(parser.parse("/courses")).toEqual({
			type: "module",
			moduleName: "courses",
			command: "about",
		});
		expect(parser.parse("/courses/aggregates")).toEqual({
			type: "module",
			moduleName: "courses",
			command: "aggregates",
		});
		expect(parser.parse("/courses/course")).toEqual({
			type: "module",
			moduleName: "courses",
