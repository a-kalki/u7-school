...
describe("CourseApiModule", () => {
  ...
  test("create-course: ADMIN создаёт курс", async () => {
    ...
    const result = await mod.handle({
      name: "create-course",
      attrs: { title: "Курс JS", description: "Описание", kind: "modules", status: Status.DRAFT },
      actorId: admin.uuid,
    });
  ...
  test("get-course: возвращает созданный курс", async () => {
    ...
    const created = await mod.handle({
      name: "create-course",
      attrs: { title: "Курс Python", description: "Описание", kind: "projects", status: Status.DRAFT },
      actorId: admin.uuid,
    }) as { uuid: string };
  ...
  test("list-courses: возвращает список курсов", async () => {
    ...
    await mod.handle({
      name: "create-course",
      attrs: { title: "Курс 1", description: "Описание", kind: "modules", status: Status.DRAFT },
      actorId: admin.uuid,
    });
  ...
  test("create-lesson: ADMIN создаёт урок", async () => {
    ...
    const course = await mod.handle({
      name: "create-course",
      attrs: { title: "Курс", description: "Описание", kind: "modules", status: Status.DRAFT },
      actorId: admin.uuid,
    }) as { uuid: string };

    const result = await mod.handle({
      name: "create-lesson",
      attrs: { courseId: course.uuid, title: "Урок 1", status: Status.DRAFT, order: 1 },
      actorId: admin.uuid,
    });
  ...
  test("create-step: ADMIN создаёт шаг", async () => {
    ...
    const result = await mod.handle({
      name: "create-step",
      attrs: { courseId: course.uuid, kind: "text", description: "Шаг 1", status: Status.DRAFT, order: 1 },
      actorId: admin.uuid,
    });
  ...
  test("create-file-metadata: ADMIN создаёт метаданные файла", async () => {
    ...
    const result = await mod.handle({
      ...
      attrs: { courseId: course.uuid, name: "test.pdf", url: "https://example.com/test.pdf", mimeType: "application/pdf" },
      actorId: admin.uuid,
    });
  ...
  test("MENTOR может создать курс", async () => {
    ...
    const result = await mod.handle({
      name: "create-course",
      attrs: { title: "Курс ментора", description: "Описание", kind: "projects", status: Status.DRAFT },
      actorId: mentor.uuid,
    });
