...
describe("FileMetadataPolicy", () => {
  describe("canCreate", () => {
    ...
    test("STUDENT не может создавать", () => {
      expect(FileMetadataPolicy.canCreate(makeActor([Role.STUDENT]))).toBe(false);
    });
  });

  describe("canRead", () => {
    test("любой пользователь может читать", () => {
      expect(FileMetadataPolicy.canRead(makeActor([Role.STUDENT]), file)).toBe(true);
      expect(FileMetadataPolicy.canRead(makeActor([Role.ADMIN]), file)).toBe(true);
    });
  });

  describe("canEdit", () => {
    test("ADMIN может редактировать", () => {
      expect(FileMetadataPolicy.canEdit(makeActor([Role.ADMIN]), file)).toBe(true);
    });

    test("MENTOR не может редактировать чужой файл", () => {
      expect(FileMetadataPolicy.canEdit(makeActor([Role.MENTOR]), file)).toBe(false);
    });

    test("STUDENT не может редактировать", () => {
      expect(FileMetadataPolicy.canEdit(makeActor([Role.STUDENT]), file)).toBe(false);
    });
  });
});
