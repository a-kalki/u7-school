...
describe("ProjectSchema", () => {
  ...
  test("принимает проект с пустым lessonIds", () => {
    const result = v.safeParse(ProjectSchema, { ...validProject, lessonIds: [] });
    expect(result.success).toBe(true);
  });
});
...
describe("CourseSchema", () => {
  ...
  test("отклоняет пустой заголовок", () => {
    const result = v.safeParse(CourseSchema, { ...validModulesCourse, title: "" });
    expect(result.success).toBe(false);
  });

  test("отклоняет пустое описание", () => {
    const result = v.safeParse(CourseSchema, { ...validModulesCourse, description: "" });
    expect(result.success).toBe(false);
  });
});
