...
describe("CoursePolicy", () => {
  ...
  describe("canRead", () => {
    test("любой может читать", () => {
      expect(CoursePolicy.canRead(makeActor([Role.STUDENT]), course)).toBe(true);
    });
  });

  describe("canEdit", () => {
    test("ADMIN может редактировать чужой курс", () => {
      expect(CoursePolicy.canEdit(makeActor([Role.ADMIN], "not-author"), course)).toBe(true);
    });

    test("автор может редактировать", () => {
      expect(CoursePolicy.canEdit(makeActor([Role.MENTOR], "author-uuid"), course)).toBe(true);
    });

    test("не-автор MENTOR не может редактировать", () => {
      expect(CoursePolicy.canEdit(makeActor([Role.MENTOR], "other-uuid"), course)).toBe(false);
    });

    test("STUDENT может редактировать свой курс", () => {
      expect(CoursePolicy.canEdit(makeActor([Role.STUDENT], "author-uuid"), course)).toBe(true);
    });

    test("STUDENT не может редактировать чужой курс", () => {
      expect(CoursePolicy.canEdit(makeActor([Role.STUDENT], "other-uuid"), course)).toBe(false);
    });
  });
});
