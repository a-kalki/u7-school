...
describe("LessonSchema", () => {
  ...
  test("отклоняет отрицательный estimatedMinutes", () => {
    const result = v.safeParse(LessonSchema, { ...validLesson, estimatedMinutes: -1 });
    expect(result.success).toBe(false);
  });

  test("отклоняет невалидный UUID в stepIds", () => {
    const result = v.safeParse(LessonSchema, { ...validLesson, stepIds: ["bad"] });
    expect(result.success).toBe(false);
  });

  test("отклоняет невалидный UUID в mentorStepIds", () => {
    const result = v.safeParse(LessonSchema, { ...validLesson, mentorStepIds: ["bad"] });
    expect(result.success).toBe(false);
  });
});
