...
export const LessonSchema = v.object({
  ...
  estimatedMinutes: v.optional(v.pipe(v.number(), v.integer("estimatedMinutes должен быть целым числом"), v.minValue(1, "estimatedMinutes должен быть положительным"))),
  stepIds: v.array(v.pipe(v.string(), v.uuid("Некорректный формат UUID stepIds"))),
  mentorStepIds: v.array(v.pipe(v.string(), v.uuid("Некорректный формат UUID mentorStepIds"))),
});
