...
describe("LessonPolicy", () => {
  ...
  describe("canRead", () => {
    test("любой пользователь может читать", () => {
      expect(LessonPolicy.canRead(makeActor([Role.STUDENT]), lesson)).toBe(true);
    });
  });
  ...
  describe("canEdit", () => {
    ...
    test("MENTOR не может редактировать напрямую", () => {
      expect(LessonPolicy.canEdit(makeActor([Role.MENTOR]), lesson)).toBe(false);
    });

    test("STUDENT не может редактировать", () => {
      expect(LessonPolicy.canEdit(makeActor([Role.STUDENT]), lesson)).toBe(false);
    });
  });
});
