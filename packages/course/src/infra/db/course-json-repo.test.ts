...
describe("CourseJsonRepo", () => {
  ...
  test("getAll возвращает все курсы", async () => {
    ...
    const c2 = makeCourse({ uuid: crypto.randomUUID(), title: "Второй" } as Partial<Course>);
  ...
  test("getAll с фильтром status", async () => {
    ...
    const published = makeCourse({ uuid: pubUuid, status: Status.PUBLISHED } as Partial<Course>);
