  ...
  describe("getAll", () => {
    ...
    describe("фильтры", () => {
      ...
      test("фильтрует по роли — только STUDENT", async () => {
        ...
        expect(result).toHaveLength(3); // user1, petrStudent, boris (у него STUDENT+MENTOR)
        expect(result.map((u) => u.name)).toEqual(
          expect.arrayContaining(["Иван", "Петр Студент", "Борис"]),
        );
      });

      test("фильтрует по роли — только MENTOR", async () => {
        const result = await repo.getAll({ role: Role.MENTOR });
        expect(result).toHaveLength(3); // user2, anna, boris
        expect(result.map((u) => u.name)).toEqual(
          expect.arrayContaining(["Петр", "Борис"]),
        );
      });
      ...
      test("ищет по имени — частичное совпадение", async () => {
        ...
        expect(result).toHaveLength(2); // "Петр" и "Петр Студент"
        expect(result.map((u) => u.name)).toEqual(
    ...
    describe("сортировка", () => {
      ...
      test("сортирует по createdAt по возрастанию", async () => {
        ...
        expect(dates).toEqual([
          ...
          "2026-05-01T12:00", // user1, user2, user3 — одинаковые даты
          "2026-05-01T12:00",
          "2026-05-01T12:00",
