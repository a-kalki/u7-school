...
| **Job (периодическое задание)** | `api/<entity>/<name>-job.ts` | `conductor/code_styleguides/skills/job.md` |
...
2. **DS:** доменный объект для одновременной работы с двумя или более агрегатами.
3. **Policy ≠ навигация:** Policy отвечает на `can?`. Структурная навигация — в Ar.
4. **Фасады:** представители модулей в других модулях, предоставляют удобный доменный api для клиента.
5. **Межмодульное:** зависимый модуль импортирует Policy/Ar/Ds/Facade вышестоящего, делегирует, передаёт минимум данных.
