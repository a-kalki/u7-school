...
### Проверка ошибок
- Тесты исключений должны проверять **конкретное сообщение**: `expect(...).toThrow("ожидаемое сообщение")`,
  а не просто `expect(...).toThrow()`.
- Для проверки типа исключения: `expect(...).toThrow(ApiException)`.
